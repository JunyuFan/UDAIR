...
class SimCLR(nn.Module):
    def __init__(
        ...
        self.forward(torch.randn(1, channels, image_size, image_size), torch.randn(1, channels, image_size, image_size))
    ...
    def forward(self, x, y):
        ...
        queries, _ = self.net(x)
        keys, _    = self.net(y)

        # queries = self.avg_pool1(x)
        # keys = self.avg_pool2(y)


        queries, keys = map(flatten, (queries, keys))
        loss = nt_xent_loss(queries, keys, temperature = self.temperature)
        # loss = contrastive_loss(queries, keys, temperature = self.temperature)
        return loss
